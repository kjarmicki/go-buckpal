...
db_migrate_up:
	docker run -v $(shell pwd)/migrations:/migrations --network host migrate/migrate:v4.15.2 -path=/migrations/ -database "mysql://root:1@tcp(localhost:${DB_PORT})/${DB_NAME}" up
